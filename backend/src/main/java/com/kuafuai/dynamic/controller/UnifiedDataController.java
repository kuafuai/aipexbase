...
public class UnifiedDataController {
    ...
    public Object handle(
        ...
        log.info("收到请求 - 应用ID: {}, 表名: {}, 方法: {}", appId, table, method);
        ...
        // 执行限流检查
        boolean allowed = rateLimitService.tryAcquire(appId);
        log.info("限流检查结果 - 应用ID: {}, 允许: {}", appId, allowed);
        
        if (!allowed) {
            // 限流触发，返回详细的限流信息
            LocalAppRateLimitService.RateLimitStatus status = rateLimitService.getStatus(appId);
