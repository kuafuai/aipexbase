...
@Component
public class WordToVideoFirstTailProcessor implements HuoshanParamProcessor {

    @Override
    public boolean match(String reqKey) {
        return StringUtils.equalsIgnoreCase(reqKey, "jimeng_i2v_first_tail_v30");
    }
    ...
    public String template(HuoshanConfig config) {
        if (StringUtils.equalsIgnoreCase(config.getAction(), "CVSync2AsyncGetResult")) {
            return "{\"req_key\":\"jimeng_i2v_first_tail_v30\",\"task_id\":\"${{task_id}}\"}";
        } else {
            return "{\"req_key\":\"jimeng_i2v_first_tail_v30\",\"image_urls\":${{image_urls}},\"prompt\":\"${{prompt}}\"}";
        }
    }
}
